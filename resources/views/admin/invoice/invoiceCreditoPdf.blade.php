...
<body>
    ...
    <div style="width:300px; float:right; margin-top:10px;height:40px;">
       ...
       <div style="font-size:30px; font-weight:bold; color:#383838;">
       {{ trans('message.transaction.credit') }}</div>
     @if($saleDataOrder!=null)
     <strong># {{$SaleCredito->reference_credit}}</strong>
     <div>{{trans('message.table.date')}}: {{formatDate($SaleCredito->credit_date)}}</div>
     <h5>{{trans('message.table.invoice_reference')}}:(#{{$factura}})</h5>
...
<div style="margin-top:40px;height:125px;">
  ...
  <table style="width:100%; border-radius:2px;  ">
    ...
    <tr style="background-color:#f0f0f0; border-bottom:1px solid #d1d1d1; text-align:center; font-size:13px;      font-weight:bold;">
      ...
      <td style="padding-right:140px;text-align:center; color: #000000" >{{ trans('message.quotation.item_description') }}</td>
      <td style="padding-right:1px;text-align:center; color: #000000">{{ trans('message.quotation.quantity') }}</td>
      <td style="padding-right:1px;text-align:center; color: #000000">{{ trans('message.quotation.price') }}({{Session::get('currency_symbol')}})</td>
      <td style="padding-right:1px;text-align:center; color: #000000">{{ trans('message.quotation.tax') }}(%)</td>
  ...
  @if(count($produtos )>0)
    ...
    <tr style="background-color:#fff; text-align:center; font-size:13px; font-weight:normal;">
      ...
      @php
        ...
        $taxAmount = (($newPrice*$produto->tax_type_id)/100);
        $taxTotal += $taxAmount;
        $subTotal += $newPrice;
