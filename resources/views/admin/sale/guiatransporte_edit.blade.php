...
<section class="content">
   <div class="row">
      <div class="col-md-12">
        ...
        <!-- /.box-header -->
        <div class="box-body">
           <h4 class="text-info">Guia de transporte # <a href="{{url('sales/view_detail_gt/'.$invoiceData->gt_no.'/')}}">{{$invoiceData->reference_gt}}</a></h4>
        <div class="clearfix"></div>
            

        <form action="{{url('sales/update_gt')}}" method="POST" id="geForm">  
        <input type="hidden" value="{{csrf_token()}}" name="_token" id="token">
        <input type="number" id="discounto" value="" hidden>
        <input type="number" id="imposto" value="" hidden>
        <input type="text" id="gt_no" name="gt_no" value="{{$invoiceData->gt_no}}" hidden>
        <div class="row">
          <!--ref-->
          <div class="col-md-3">
              <div class="form-group">
                  <label for="exampleInputEmail1">{{ trans('message.table.reference') }}<span class="text-danger"> *</span></label>
                     <input id="reference_no" class="form-control" name="ge_ref" value="GT-{{ sprintf("%04d", $invoice_count+1)}}/<?php echo $parte_ano;?>" type="text" readonly>
            ...
            <div class="col-md-3">
               <div class="form-group">
                ...
                @foreach($customerData  as $key => $data)
                  @if($invoiceData->debtor_no_gt==$data->debtor_no)
        ...
        <div class="row">
            ...
            <div class="col-md-3">
              <div class="form-group">
                <label>{{ trans('message.form.data_trans') }}<span class="text-danger"> *</span></label>
                  <input class="form-control" id="datepicker" type="text" name="gt_date" value="{{$invoiceData->gt_date}}" readonly>
              </div>
        ...
        <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                    ...
                    <textarea placeholder="{{ trans('message.table.description') }} ..." rows="6" class="form-control" name="comments">{{$invoiceData->comments}} </textarea>
                </div>
                <a href="{{url('/sales/guiatransporte')}}" class="btn btn-info btn-flat">{{ trans('message.form.cancel') }}</a>
  ...
  $("#lista_invoice").on('change', function(){
      ...
      $.ajax({
       ...
       console.log(e);
                ...
                // Adicionando elementoos na ordem  de adicao////quantity
