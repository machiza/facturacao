...
<body>
  ...
  <div style="height:150px">
    <div style="width:70%; float:left; font-size:15px; color:#383838; font-weight:400;">
      <div style="font-size:20px;"><strong>{{ trans('message.report_pdf.sales_report') }}</strong></div>
      <div>{{ trans('message.report_pdf.print_date') }} : {{formatDate(date('d-m-Y'))}}</div>
       @if($type == 'custom')
       <div>{{ trans('message.form.date') }} : {{ $date_range }}</div>
       @endif

       <div>{{ trans('message.report.product') }} : {{isset($product) && ($product == 'all') ? 'All' : getItemName($product)}}</div>
       <div>{{ trans('message.form.customer') }} : {{isset($customer) && ($customer == 'all') ? 'All' : getCustomer($customer)}}</div>
       <div>{{ trans('message.form.location') }} : {{isset($location) && ($location == 'all') ? 'All' : getDestinatin($location)}}</div><br/><br/>
 ...
 <tr style="background-color:#f0f0f0; border-bottom:1px solid #d1d1d1; text-align:center; font-size:13px; font-weight:bold;">
   ...
   <td>{{ trans('message.report.sales_volume') }}</td>
