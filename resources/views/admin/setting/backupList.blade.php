            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                @if(Helpers::has_permission(Auth::user()->id, 'download_db_backup'))
                      <a  title="Download" href="{{URL::to('/storage/laravel-backups/'.$data->name)}}"  class="btn btn-xs btn-info edit_unit" id="{{$data->id}}" ><span class="fa fa-download"></span></a> &nbsp;
                @endif
             
                @if(Helpers::has_permission(Auth::user()->id, 'add_db_backup'))
                 <form method="POST" action="{{ url("backup/delete/$data->id") }}" accept-charset="UTF-8" style="display:inline">
                    {!! csrf_field() !!}
                    <button class="btn btn-xs btn-danger" type="button" data-toggle="modal" data-target="#confirmDelete" data-title="Backup Delete" data-message="Are you sure to delete this backup ?">
