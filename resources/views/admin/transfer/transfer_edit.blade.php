...
@section('content')
    ...
    <div class="box">
      <div class="box-body">
        <h4 class="text-info text-center">{{ trans('message.transaction.edit_transfer') }}</h4>
          <!-- form start -->
        <form action="{{ url('transfer/update') }}" method="post" id="transfer" class="form-horizontal">
        
        <input type="hidden" value="{{csrf_token()}}" name="_token" id="token">
        <input type="hidden" value="{{$transferInfo->id}}" name="id" id="id">
        ...
        <div class="col-md-12">
          <div class="form-group">
            ...
            <div class="col-sm-6">
               <select class="form-control select2" name="source" id="source" disabled>
                <option value="">{{ trans('message.transaction.chose_account') }}</option>
                @foreach($accounts as $acc_no=>$acc_name)
                  <option value="{{$acc_no}}" <?= ($acc_no==$transferInfo->id) ? 'selected' : ''?>>{{$acc_name}}</option>
                @endforeach
                </select>
            </div>
          </div>         
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
            <input type="text" class="form-control" id="trans_date" name="trans_date" value="{{formatDate($transferInfo->trans_date)}}">
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
              <input type="text" placeholder="{{ trans('message.table.description') }}" class="form-control" id="description" name="description" value="{{$transferInfo->description}}">
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
              <input type="text" placeholder="{{ trans('message.table.amount') }}" class="form-control" id="amount" name="amount" min="0" value="{{abs($transferInfo->amount)}}" readonly>
            ...
            <div class="col-sm-6">
               <select class="form-control select2" name="payment_method" id="payment_method">
                ...
                @foreach($payment_methods as $method_id=>$method_name)
                  <option value="{{$method_id}}" <?= ($method_id == $transferInfo->payment_method ) ? 'selected' : '' ?> >{{$method_name}}</option>
                @endforeach
                </select>
            </div>
          </div>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-6">
              <input type="text" placeholder="{{ trans('message.table.reference') }}" class="form-control" id="reference" name="reference" value="{{$transferInfo->reference}}">
            </div>
          </div>
