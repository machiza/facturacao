...
class PaymentController extends Controller
{
    ...
    */
    ...
    public function editPayment(Request $request){
     ...
     public function reportFilter($dataI,$dataF,$cust) {
      ...
      if($customer != NULL) {
        $data['paymentList'] = DB::table('sales_cc')
                            ...
                            ->leftjoin('payment_terms','payment_terms.id','=','sales_cc.payment_type_id_doc')
                            ->leftjoin('sales_orders','sales_orders.reference','=','sales_cc.reference_doc')
                            ->select('sales_cc.*','debtors_master.name','payment_terms.name as pay_type',
                                    'sales_orders.order_no as invoice_id','sales_orders.order_reference_id as order_id')
                            ->where('debtors_master.debtor_no','=',$customer)
                            ->where('sales_cc.ord_date_doc','>=',$from)
                            ->where('sales_cc.ord_date_doc','<=',$to)
                            ->orderBy('sales_cc.idcc','DESC')->where('payment_type_id_doc','2')
                            ->get();
      } else {
        $data['paymentList'] = DB::table('sales_cc')
