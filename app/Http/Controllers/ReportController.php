...
//$data['profit'],1);
    ...
    */
    ...
    //novos metodos by @shady
    public function venda_por_stock(){
        $data['type'] = 'all';
        $data['location_id'] = 'all';
        
        $data['customerList'] = DB::table('debtors_master')->where(['inactive'=>0])->get();
        $data['menu']     = 'report';
        $data['sub_menu'] = 'report/inventory-sold';

        $fromDate = DB::table('sales_order_details')->select('ord_date')
        ->orderBy('ord_date','asc')->first();
        $toDate = DB::table('sales_order_details')->select('ord_date')->orderBy('ord_date','desc')->first();

      $data['from'] = $from = isset($fromDate->ord_date) ? formatDate($fromDate->ord_date): formatDate(date("Y-m-d", strtotime("-1 months")));
      $data['to'] = $to = isset($toDate->ord_date) ? formatDate($toDate->ord_date) : formatDate(date('Y-m-d'));


        if(isset($_GET['btn'])){
            // $user = $_GET['customer']; 24-08-2018  28-08-2018
            $from = date('Y-m-d',strtotime($_GET['from']));
            $to   = date('Y-m-d',strtotime($_GET['to']));  
           ...
           $data['itemList'] =  $this->report->getSales_sold($from,$to);
           $data['Quantity'] =  $this->report->getSales_sold_quantidade($from,$to);
           $data['venda'] =  $this->report->getSales_sold_venda($from,$to);
           $data['custo'] =  $this->report->getSales_sold_custo($from,$to);
          
           $from = DB::table('sales_orders')->select('ord_date')->where('trans_type',SALESINVOICE)->orderBy('ord_date','asc')->first();
           $to = DB::table('sales_orders')->select('ord_date')->where('trans_type',SALESINVOICE)->orderBy('ord_date','desc')->first();

          if(!empty($from) && !empty($to)){
           $data['from'] = formatDate($fromDate->ord_date);
           $data['to'] = formatDate($toDate->ord_date);
          }
   ...
   public function venda_por_stock_pdf(){

        return 'Relatorio pdf';
  }


  public function inventoryExistente(){
        
        $data['type'] = 'all';
        $data['location_id'] = 'all';
        $data['menu']     = 'report';
        $data['sub_menu'] = 'report/inventory';
        ...
        foreach ($itemList as $key => $item) {
            ...
            $qtyOnHand += $item->available_qty;
            $qtyHand=$qtyHand+($item->available_qty);
        ...
        return view('admin.report.new.inventory_stock_on_hand', $data);
  ...
  public function inventoryGeneral(){
        ...
        $data['location_id'] = 'all';
        $data['menu']     = 'report';
        $data['sub_menu'] = 'report/inventory-general';
        ...
        //$fromDate = DB::table('stock_moves')->select('tran_date')->orderBy('tran_date','asc')->first();
        $toDate = DB::table('stock_moves')->select('tran_date')->orderBy('tran_date','desc')->first();
        ...
        if(isset($_GET['btn'])) {
           ...
           $data['itemList'] = $itemList = $this->report->getInventoryStockOnHand_date($type,$location,$to); 
        }else{
            $data['itemList'] = $itemList = $this->report->getInventoryStockOnHand_date($data['type'],$data['location_id'],$to1);  
           }
        ...
        return view('admin.report.new.inventory_general', $data);
