...
class ItemController extends Controller
{
    ...
    public function create($tab)
    {
        ...
        $data['brandData'] = DB::table('brand')->get();
    ...
    public function store(Request $request)
    {
        ...
        $data['item_type_id'] = $request->tipo_servico;
        ...
        if (!empty($id)) {
            ...
            $data2['brand_id'] = $request->brand_id;
     ...
     */
     ...
     public function updateSalePrice(Request $request)
     {
         ...
         DB::table('sale_prices')
            ->where('id', $id)
            ->update(['price' => $request->price, 'inclusao_iva'=>$request->inclusao_iva, 'discounto'=>$request->discounto ]);
        ...
        if (isset($pic)) {
          ...
          $data['item_image'] = $filename;
           ...
           DB::table('item_code')->where('id', $request->id)->update($data);
            ...
            $stockMaster['brand_id'] = $request->brand_id;
    ...
    public function downloadCsv($type)
    {
        if ($type == 'xls' ) {
          ...
          if($itemdata!=null){
            ...
            foreach ($itemdata as $key => $value) {
               $csvdata[$key]['Item_Brand'] = $value->item_brand;
              // $csvdata[$key]['Item_Brand'] = $value->stock_id;
                $csvdata[$key]['Item_ID'] = $value->stock_id;
                $csvdata[$key]['Item_Name'] = $value->item_name;
                    //$csvdata[$key]['Long Description'] = $value['long_description'];
                $csvdata[$key]['Item_Category'] = $value->category;
                     //$csvdata[$key]['Unit'] = $value['units'];
                    //$csvdata[$key]['Tax Type'] = $value['tax_type'];
                $csvdata[$key]['Purchase_Price'] = $value->purcashe_price;
                $csvdata[$key]['Retail_Price'] = $value->retail_price;
                    ...
                    \Session::flash('fail',trans('Sem dados por actualizar'));
                    return redirect()->intended('item');
          }
        ...
        if( $type == 'sample' ) {
            $csvdata[0]['Item_Brand'] = 'DELL'; 
            $csvdata[0]['Item_ID'] = 'DELX1'; 
            $csvdata[0]['Item_Name'] = 'Dell desktop computer';
            $csvdata[0]['Long_Description'] = 'This is a destop computer';
            $csvdata[0]['Item_Category'] = 'Computer';
            $csvdata[0]['Unit'] = 'pc';
            $csvdata[0]['Tax_Type'] = 'Normal';
            $csvdata[0]['Purchase_Price'] = '50';
            $csvdata[0]['Retail_Price'] = '60';
            

            $type = 'xls';
        }

        return Excel::create('DemoImportStock'.time().'', function($excel) use ($csvdata) {
            
            $excel->sheet('primeira', function($sheet) use ($csvdata)
            {
    ...
    public function importCsv(Request $request)
    {
        ...
        if (Input::hasFile('import_file')) {
           ...
           //d($csv,1);
            $unMatch = [];
            $header = array("Item ID", "Item Name", "Long Description", "Item Category", "Unit", "Tax Type", "Purchase Price", "Retail Price");
            ...
            foreach ($csv as $key => $value) {
                if($key != 0) {
                   ...
                   // $data[$key]['long_description'] = $value[2];
                    $data[$key]['item_category'] = $value[2];
                   // $data[$key]['unit'] = $value[4];
                    //$data[$key]['tax_type'] = $value[5];
                    $data[$key]['purch_price'] = $value[3];
                    $data[$key]['retail_price'] = $value[4];
                }
            }
           ...
           $data = Excel::load($path, function($reader) {
            ...
            if(!empty($data) && $data->count()){
                foreach ($data as $key => $value) { 
                 ...
                 $this->Salvar($value->item_brand,$value->item_id,$value->item_name,$value->long_description,$value->item_category,$value->unit,$value->purchase_price,$value->retail_price);
   ...
   public function Salvar($item_brand,$stock_id,$description,$long_description,$category1,$units,$priceCompra,$priceVenda){

    $brand_id=1;
    $category_id=1;


    $category = DB::table('stock_category')->where('description','=',$category1)->first();
    $brand = DB::table('brand')->where('description','=',$item_brand)->first();
    ...
    $category_id= $category->category_id;                    
        ...
        $data['stock_id'] = strtoupper($stock_id);
        ...
        if (!empty($id)) {
            Session::put('stock_id', strtoupper($stock_id));

            $data2['stock_id'] = strtoupper($stock_id);
            $data2['description'] = $description;
            $data2['long_description'] = $long_description;
            $data2['units'] = $units;
            $data2['tax_type_id'] = 3;
            $data2['category_id'] = $category_id;
            $data2['brand_id'] = $category_id;
           

            DB::table('stock_master')->insert($data2);

            $data3[0]['stock_id'] = strtoupper($stock_id);
            $data3[0]['sales_type_id'] = 1;
            $data3[0]['price'] = $priceVenda;
            $data3[0]['curr_abrev'] = 'MT';


            $data3[1]['stock_id'] = strtoupper($stock_id);
            $data3[1]['sales_type_id'] = 2;
            $data3[1]['price'] = $priceVenda;
            $data3[1]['curr_abrev'] = 'MT';

            DB::table('sale_prices')->insert($data3);

            $purchaseInfos['stock_id'] = strtoupper($stock_id);
            $purchaseInfos['price'] = $priceCompra;
            DB::table('purchase_prices')->insert($purchaseInfos);
