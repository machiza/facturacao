...
class CategoryController extends Controller
{
     ...
     public function store_brand(Request $request)
    {
        ...
        */
        ...
        $id = (new Category)->createBrand($data);

        if (!empty($id)){

            \Session::flash('success',trans('message.success.save_success'));
            return redirect()->intended('item-brand');
        } else {
    ...
    {
        $id = $_POST['id'];
        $brandData =DB::table('brand')->where('id','=',$id)->first();
        $return_arr['id'] = $brandData->id;
        $return_arr['description'] = $brandData->description;
    ...
    {    
        ...
        $id = (new Category)->updateBrand($id, $data);
    ...
    {
        if (isset($id)) {
            $record = \DB::table('brand')->where('id', $id)->first();
            if ($record) {
    ...
    public function downloadCsv($type)
    {
      ...
      public function downloadxls_brand($type)
    {
        if ($type == 'xls' ) {
            $categorydata = (new Category)->getBrands();
            foreach ($categorydata as $key => $value) {
        ...
        if( $type == 'sample' ) {
            $data[0]['Name'] = 'Brand Name'; 
            $type = 'xls';
        }

        return Excel::create('Maracas_folha'.time().'', function($excel) use ($data) {
            
            $excel->sheet('primeira', function($sheet) use ($data)
            {
                $sheet->fromArray($data);
            });
     ...
     public function import_brand()
    {
        ...
        return view('admin.brand.brand_import', $data);
    }
    ...
    public function importCsv(Request $request)
    {
      ...
      public function importxls_brand(Request $request)
    {
           ...
           $data = Excel::load($path, function($reader) {
            ...
            if(!empty($data) && $data->count()){
                foreach ($data as $key => $value) {
                 ...
                 $existencia=  \DB::table('brand')->where('description',$value->name)->first();
                    if($existencia==null){   
                      $id = (new Category)->createBrandExcel($value->name);
                    }else{
