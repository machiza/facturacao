...
class Report extends Model
{
    ...
    public function getInventoryStockOnHand_date($type,$location,$date)
    {
    	if($type=='all' && $location=='all'){
    	$data = DB::select(DB::raw("SELECT item.stock_id,item.description,COALESCE(sp.price,0) as retail_price,COALESCE(sm.qty,0) as available_qty,COALESCE(pod.received_qty,0) as received_qty,COALESCE(pod.price,0) as cost_amount 
			FROM(SELECT * FROM stock_master WHERE inactive=0 AND deleted_status = 0)item
			...
			LEFT JOIN(SELECT stock_id,sum(qty)as qty FROM stock_moves  WHERE tran_date <= '$date'GROUP BY stock_id)sm
			ON sm.stock_id = item.stock_id
    	...
    	$data = DB::select(DB::raw("SELECT item.stock_id,item.description,COALESCE(sp.price,0) as retail_price,COALESCE(sm.qty,0) as available_qty,COALESCE(pod.received_qty,0) as received_qty,COALESCE(pod.price,0) as cost_amount 
			...
			LEFT JOIN(SELECT stock_id,sum(qty)as qty FROM stock_moves WHERE loc_code = '$location' and  tran_date <= '$date' GROUP BY stock_id)sm
			ON sm.stock_id = item.stock_id
    	...
    	$data = DB::select(DB::raw("SELECT item.stock_id,item.description,COALESCE(sp.price,0) as retail_price,COALESCE(sm.qty,0) as available_qty,COALESCE(pod.received_qty,0) as received_qty,COALESCE(pod.price,0) as cost_amount 
			...
			LEFT JOIN(SELECT stock_id,sum(qty)as qty FROM stock_moves WHERE  tran_date <= '$date' and  loc_code = '$location' GROUP BY stock_id)sm
			ON sm.stock_id = item.stock_id
...
$data = DB::select(DB::raw("SELECT sales_orders.ord_date,count(infos.order_no) as total_order,SUM(infos.qty) as qty,SUM(infos.purchase) as purchase,SUM(infos.sale) as sale,infos.stock_id FROM sales_orders
    ...
    //novos relatrios done by sh@dy
    public function getSales_sold($from,$to){ 	
		
		return $data = DB::table('sales_order_details')
						->where('sales_order_details.is_inventory','=',1)
						->whereBetween('sales_order_details.ord_date', [$this->convert($from),$this->convert($to)])
                        ->select('created_at','stock_id','description','unit_price as price','cost_price',DB::raw('SUM(quantity) as quantidade'))
                        ->groupBy('stock_id')
                        ->get();	
	}


	public function getSales_sold_quantidade($from,$to){ 	
		$datas = DB::table('sales_order_details')
						->where('sales_order_details.is_inventory','=',1)
						->whereBetween('sales_order_details.ord_date', [$this->convert($from),$this->convert($to)])
                        ->select('created_at','stock_id','description','unit_price as price','cost_price',DB::raw('SUM(quantity) as quantidade'))
                        ->groupBy('stock_id')
              ...
              foreach ($datas as $data) {
              	$quantity=$quantity+$data->quantidade;          
              }       
              return $quantity;  

	}

	public function getSales_sold_custo($from,$to){ 	
		$datas = DB::table('sales_order_details')
						->where('sales_order_details.is_inventory','=',1)
						->whereBetween('sales_order_details.ord_date', [$this->convert($from),$this->convert($to)])
                        ->select('created_at','stock_id','description','unit_price as price','cost_price',DB::raw('SUM(quantity) as quantidade'))
			  ...
			  $quantity=0;
              foreach ($datas as $data) {
              	$quantity=$quantity+($data->quantidade*$data->cost_price);          
              }       
              return $quantity;  
	...
	public function getSales_sold_venda($from,$to){ 
		...
		$datas = DB::table('sales_order_details')
			  ...
			  $quantity=0;
              foreach ($datas as $data) {
              	$quantity=$quantity+($data->quantidade*$data->price);          
              }       
              return $quantity;  
