...
//d($conditions,1);
    ...
    public function getSalseData($id)
    {
      $data= DB::table('sales_credito')
                                        ->leftjoin('sales_order_details','sales_credito.credit_no','=','sales_order_details.order_no')
                                        ->leftJoin('item_code', 'sales_order_details.stock_id', '=', 'item_code.stock_id')
                                        ->leftJoin('item_tax_types', 'item_tax_types.id','=','sales_order_details.tax_type_id')
                                        ->select('sales_order_details.*','item_code.id as item_id','item_tax_types.tax_rate')->where('credit_no',$id)->where('tipo_operacao','=','credito')
